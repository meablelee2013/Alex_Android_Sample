...
public class StartupProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        try {
            List<AndroidStartup<?>> startups = StartupInitializer.discoverAndInitialize(getContext(), getClass().getName());
            new StartupManager.Builder().addAllStartup(startups).build(getContext()).start().await();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return true;
